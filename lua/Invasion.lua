...
function INV_mensaje(_tipo, _texto, _forHelis, _titulo, _groupID)
    ...
    elseif (_groupID ~= false) then
        ...
        for i, _group in pairs(coalition.getGroups(coalition.side.BLUE,
                                                   Group.Category.HELICOPTER)) do
            if _group ~= nil then
                local _groupHELI = Group.getID(_group)
                trigger.action.outSoundForGroup(_groupHELI, _snd)
                trigger.action.outTextForGroup(_groupHELI,
                                               _msg .. _texto .. "\n", 10, true)
            end
        end
    end
...
function checkAlive(_groupName)
    ...
    for j, _unit in pairs(_group:getUnits()) do
        if _unit ~= nil then return _unit end
    end
    return nil
end
...
function checkAliveNumber(_groupName)
    ...
    local i = 0
    for j, _unit in pairs(_group:getUnits()) do
        if _unit ~= nil then if (_unit:getLife() >= 1) then i = i + 1 end end
    end
    return i
end
...
function getUnitFromMission(_unidad)
    if type(_unidad) == "string" then
        ...
        for _, _unitName in pairs(_unidad) do return _unitName end
    end
end
...
function continueGroup(_groupName)
    local _group = Group.getByName(_groupName)
    if (_group ~= nil) then trigger.action.groupContinueMoving(_group) end
end
...
----------------------------
-- MENUS OBJ --
function insertarMision(_txt1, _units, _type, _percent, _txt3, _complete,
                        _trigger)
    ...
    if (_trigger ~= false) then invasion_AG[_numArray].Trigger = _trigger end
...
function unidadesActivas(_tipo, _objetivo, _squadron)
    ...
    elseif (_tipo == "cap") then
        _unidadesActivas = _squadron.ResourceCount +
                               totalAirplaneGroup(_objetivo)
    end
...
function misionesDinamicas(_groupID)
    ...
    for j, _menu in pairs(invasion_AG) do
        if (_menu.Dinamico == true and _menu.Realizado == false) then
            ...
            _msg =
                _msg .. "[" .. getCordenadas(_unitName, _unitTipo) .. "] (" .. j ..
                    ") " .. _menu.NombreObjetivo .. " - " .. _menu.TextoObjetivo
            i = i + 1
        end
    end
    if (i == 0) then
...
function inicioMenu()
    for j, _menu in pairs(invasion_AG) do
        if (_menu.Dinamico == false) then
                    ...
                    if (type(_menu.TextoObjetivo) == "string") then
                        ...
                        for j, _unitName in ipairs(_menu.Unidades) do
                            ...
                            if (_menu.UnidadesTipo[j] == "cap") then
                                _msg = _msg ..
                                           unidadesActivas(
                                               _menu.UnidadesTipo[j], _unitName,
                                               _menu.MooseSquadron[j])
                            else
                                _msg = _msg ..
                                           unidadesActivas(
                                               _menu.UnidadesTipo[j], _unitName) ..
            ...
            trigger.action.markToCoalition(j, _menu.NombreObjetivo .. "\n" ..
                                               _menu.TextoObjetivo,
                                           getVec3FromMission(_unitName,
                                                              _unitTipo),
                                           coalition.side.BLUE, true)
        end
    end
...
function menuDinamico(_groupID)
    if (menusDinamicos[_groupID] == nil) then
        menusDinamicos[_groupID] = missionCommands.addCommandForGroup(_groupID,
                                                                      "MISIONES DINAMICAS->",
                                                                      nil,
                                                                      misionesDinamicas,
                                                                      _groupID)
    ...
    if (type(_unidad) == "string") then
        ...
        if (_tipo == "grupo") then
            ...
            if (_porcentaje == 100) then
                if (_unidadesPercent == 0) then return true end
            else
        ...
        elseif (_tipo == "cap") then
            local _totalSquadron = _squadron.ResourceCount +
                                       totalAirplaneGroup(_unidad)
        ...
        for j, _unitName in ipairs(_unidad) do
            if (missionCompleteUnit(_tipo, _unitName, _porcentaje, _squadron) ==
...
local function missionComplete(_objetivo)
    if (type(_objetivo.Unidades) == "string") then
        ...
        if (_objetivo.UnidadesTipo == "cap") then
            return missionCompleteUnit(_objetivo.UnidadesTipo,
                                       _objetivo.Unidades, _objetivo.Porcentaje,
                                       _objetivo.MooseSquadron)
        else
            return missionCompleteUnit(_objetivo.UnidadesTipo,
                                       _objetivo.Unidades, _objetivo.Porcentaje)
        end
        ...
        for j, _unitName in ipairs(_objetivo.Unidades) do
            ...
            if (_unitTipo == "grupo") then
                if (missionCompleteUnit(_unitTipo, _unitName,
                                        _objetivo.Porcentaje) == false) then
                    return false
                else
                ...
                if (_unitTipo.UnidadesTipo == "cap") then
                    if (missionCompleteUnit(_unitTipo, _unitName,
                                            ...
                                            _objetivo.MooseSquadron[j]) == true) then
                        _itemComplete = _itemComplete + 1
                    end
                else
                    if (missionCompleteUnit(_unitTipo, _unitName,
                                            _objetivo.Porcentaje) == true) then
                        _itemComplete = _itemComplete + 1
                    end
                end
            end
        ...
        elseif (_objetivo.Porcentaje < 100) then
            if (_objetivo.Porcentaje <=
                ((1 - (_itemComplete / _totalItems)) * 100)) then
                return true
            else
                return false
            end
                ...
                if (missionComplete(_menu) == true) then
                    ...
                    if (_menu.Dinamico == false) then
                        missionCommands.removeItemForCoalition(
                            coalition.side.BLUE, {_nombreMision})
                    else
    ...
    elseif (_eventDCS.id == world.event.S_EVENT_BIRTH) then
        ...
        if _plrName ~= nil then
            ...
            timer.scheduleFunction(function()
                INV_mensaje(4, _msg, false,
                            "Bienvenido " .. _plrName .. " - Grupo " ..
                                Group.getName(Unit.getGroup(_eventDCS.initiator)),
                            _groupID)
            end, {}, timer.getTime() + 2)
        end
    end
